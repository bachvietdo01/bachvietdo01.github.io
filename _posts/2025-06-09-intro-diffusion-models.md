...
Here, $X_t \sim p_t(\cdot)$ traces the marginal probability path, $u_t(X_t)$ is the deterministic vector field, $\sigma_t^2$ is the diffusion coefficient, and $W_t$ denotes Brownian motion. Personally, I find the SDE perspective more intuitive and rigorous. Formally, the solution $\\{ X_t \\}$ to the SDE $(1)$ is the **underdamped Langevin diffusion process** (see [Cheng et al., 2018](https://arxiv.org/pdf/1707.03663))
...
$$
\begin{aligned}
X_{t+h} = X_t + h\cdot u_t + h \cdot \frac{\sigma_t^2}{2} \nabla_x \log p_t(X_t) + \sqrt{h} \cdot \sigma_t \cdot N(\cdot \mid 0, I).
\end{aligned}
$$
