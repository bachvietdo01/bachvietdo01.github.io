...
As in Flow Matching setup, given a data sample $z_1, z_2, \ldots, z_n$, we first construct a conditional probability path $p_0(\cdot \mid z), \ldots, p_t(\cdot \mid z), \ldots, p_1(\cdot \mid z)$. A common choice in the literature is Conditional Gassian path $p_t(\cdot | z) = N(\cdot \mid \alpha_t z , \quad  \beta_t^2 I)$ where $\alpha_t$ and $\beta_t$ are noise schedulers and $\alpha_t \to 1$ and $\beta_t^2 \to 0$ as $t \to 1$.
