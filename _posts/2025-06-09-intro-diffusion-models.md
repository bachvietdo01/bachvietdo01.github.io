...
[Flow matching](https://bachvietdo01.github.io/intro-to-flow-matching-models) constructs a probability path $\\{ X_t \\}$ with time index $t \in [0,1]$ where $X_0 \sim p_{\text{init}}(\cdot)$ and $X_1 \sim p_{\text{data}}(\cdot)$ where the trajectories $\\{ X_t \\}$ is the solution to the following ODE,
