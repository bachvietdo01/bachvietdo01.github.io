...
**Flow Matching** (Lipman et al., 2023) is a powerful class of generative models behind systems like [Stable Diffusion 3](https://stability.ai/news/stable-diffusion-3) and [Meta Movie Gen](https://ai.meta.com/research/movie-gen/), capable of generating highly realistic images and videos. This gentle introduction covers the core ideas behind Flow Matching and walks through a complete example of using Flow Matching models to sample from an 11-component multivariate Gaussian mixture, starting from an initial standard Gaussian distribution. 
...
## How to sample $p_{\text{data}}$ from a learned vector field ?
...
$$
\begin{align}
p_t(x \mid z) = N(\cdot \mid \alpha_t z , \beta_t^2 \cdot I)
\end{align}
$$
...
The flow solution to $\frac{d}{dt} X_t = u_t^{\text{target}} (X_t)$ can be shown to describe the probabilistic path $X_0, \ldots, X_t, \ldots, X_1$, where $X_1 \sim p_{\text{data}}$. This key result is a direct implication of the continuity equation, which is a special case of the [**Fokker–Planck equation**](https://en.wikipedia.org/wiki/Fokker%E2%80%93Planck_equation). The proof can be found in Theorem 1 of Lipman et al., 2023.
...
### Step 1: build the Gaussian Conditional Probability Path
